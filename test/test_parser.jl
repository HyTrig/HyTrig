...
include("../src/parsers/ast_to_logic.jl")

# test expressions

ast::Union{ASTNode, Nothing} = _parse_tokens(tokenize("", Bindings([], [], [])))
@test isnothing(ast)

ast = _parse_tokens(tokenize("a+b", Bindings([], [], ["a", "b"])))
@test ast == ExpressionBinaryOperation("+", VariableNode("a"), VariableNode("b"))
@test to_string(ast) == "(a)+(b)"
@test to_logic(ast) == Add(Var(:a), Var(:b))
@test ast == _parse_tokens(tokenize("(a+b)", Bindings([], [], ["a", "b"])), expression)
@test ast == _parse_tokens(tokenize("((a)+b)", Bindings([], [], ["a", "b"])), constraint)
@test ast == _parse_tokens(tokenize("(a+(b))", Bindings([], [], ["a", "b"])), state)
@test ast == _parse_tokens(tokenize("((a)+(b))", Bindings([], [], ["a", "b"])), strategy)
@test ast == _parse_tokens(tokenize("(a)+(b)", Bindings([], [], ["a", "b"])))
@test_throws ParseError("Cannot parse tokens between 'a' and '('.") _parse_tokens(tokenize("a(+)b", Bindings([], [], ["a", "b"])))
@test_throws ParseError("Cannot parse tokens between 'a' and '('.") _parse_tokens(tokenize("a()+b", Bindings([], [], ["a", "b"])))
@test_throws ParseError("Cannot parse tokens between '(a)+(b)' and ')'.") _parse_tokens(tokenize("a+b)", Bindings([], [], ["a", "b"])))

ast = _parse_tokens(tokenize("-a", Bindings([], [], ["a"])))
@test ast == ExpressionUnaryOperation("-", VariableNode("a"))
@test to_string(ast) == "-(a)"
@test to_logic(ast) == Neg(Var(:a))
@test ast == _parse_tokens(tokenize("-(a)", Bindings([], [], ["a"])), expression)
@test ast == _parse_tokens(tokenize("(-(a))", Bindings([], [], ["a"])), constraint)
@test_throws ParseError("Cannot parse tokens between '(' and '-'.") _parse_tokens(tokenize("(-)(a)", Bindings([], [], ["a"])))

ast = _parse_tokens(tokenize("b+(-a)", Bindings([], [], ["a", "b"])))
...
@test ast == _parse_tokens(tokenize("b+ -a", Bindings([], [], ["a", "b"])), expression)
@test ast == _parse_tokens(tokenize("(b+(-a))", Bindings([], [], ["a", "b"])), constraint)
@test_throws TokenizeError("'+-' is an invalid sequence of symbols.") _parse_tokens(tokenize("a+-b", Bindings([], [], ["a", "b"])))

ast = _parse_tokens(tokenize("a+b-c", Bindings([], [], ["a", "b", "c"])))
...
ast = _parse_tokens(tokenize("a*sin(b)", Bindings([], [], ["a", "b"])))
...
ast = _parse_tokens(tokenize("cot(0)/10", Bindings([], [], [])))
...
@test ast == _parse_tokens(tokenize("(cot(((0)))/(10))", Bindings([], [], [])), expression)

ast = _parse_tokens(tokenize("x + y * z", Bindings([], [], ["x", "y", "z"])))
...
@test ast == _parse_tokens(tokenize("x + (y * z)", Bindings([], [], ["x", "y", "z"])), expression)
@test ast == _parse_tokens(tokenize("x + (y) * z", Bindings([], [], ["x", "y", "z"])), constraint)
...
ast = _parse_tokens(tokenize("-y * z", Bindings([], [], ["y", "z"])))
...
@test ast == _parse_tokens(tokenize("-(y) * z", Bindings([], [], ["y", "z"])), expression)
@test ast != _parse_tokens(tokenize("-(y * z)", Bindings([], [], ["y", "z"])), constraint)

ast = _parse_tokens(tokenize("x ^ y - z * 10", Bindings([], [], ["x", "y", "z"])))
...
ast = _parse_tokens(tokenize("min(max(x, y), z)", Bindings([], [], ["x", "y", "z"])))
...
ast = _parse_tokens(tokenize("x < y + 10", Bindings([], [], ["x", "y"])))
...
@test ast == _parse_tokens(tokenize("x < (y + 10)", Bindings([], [], ["x", "y"])), constraint)
@test ast == _parse_tokens(tokenize("(x < (y + 10))", Bindings([], [], ["x", "y"])), state)
@test ast == _parse_tokens(tokenize("(x) < (y + 10)", Bindings([], [], ["x", "y"])))
@test ast == _parse_tokens(tokenize("((x) < (y + 10))", Bindings([], [], ["x", "y"])))
@test_throws ParseError("Cannot parse tokens between '(x)<(y)' and '+'.") _parse_tokens(tokenize("(x < y) + 10", Bindings([], [], ["x", "y"])))

ast = _parse_tokens(tokenize("true && x < y", Bindings([], [], ["x", "y"])))
...
@test ast == _parse_tokens(tokenize("(true) && (x < y)", Bindings([], [], ["x", "y"])), constraint)
@test_throws ParseError("Cannot parse tokens between '(' and 'true'.") _parse_tokens(tokenize("(true && x) < y", Bindings([], [], ["x", "y"])))

ast = _parse_tokens(tokenize("x < 10 && false", Bindings([], [], ["x"])))
...
ast = _parse_tokens(tokenize("true || false && false", Bindings([], [], [])))
...
ast = _parse_tokens(tokenize("true && loc", Bindings([], ["loc"], [])))
...
ast = _parse_tokens(tokenize("true || false || loc1 && loc2", Bindings([], ["loc1", "loc2"], [])))
...
ast = _parse_tokens(tokenize("<<a,b>> F true", Bindings(["a", "b"], [], [])))
@test ast == Quantifier(false, false, Agents(false, AgentList(["a", "b"])), ConstraintConstant(true))
@test to_string(ast) == "<<a,b>>F(true)"
@test ast == _parse_tokens(tokenize("(<<a,b>> F (true))", Bindings(["a", "b"], [], [])), strategy)
@test_throws ParseError("Cannot parse tokens between '<<' and '('.") _parse_tokens(tokenize("<<(a,b)>> F true", Bindings(["a", "b"], [], [])))
@test_throws ParseError("Cannot parse tokens between '(' and '<<'.") _parse_tokens(tokenize("(<<(a),(b)>> F (true))", Bindings(["a", "b"], [], [])))

ast = _parse_tokens(tokenize("<<a>> F x>5 and y<10", Bindings(["a"], [], ["x", "y"])))
...
@test ast == _parse_tokens(tokenize("((<< a >> F x>5) and (y<10))", Bindings(["a"], [], ["x", "y"])))
@test ast == _parse_tokens(tokenize("((<<a>> F x>5) and (y<10))", Bindings(["a"], [], ["x", "y"])))

ast = _parse_tokens(tokenize("<< >> F x>5 && y<10", Bindings([], [], ["x", "y"])))
@test ast == Quantifier(
    false,
    false,
...
@test ast == _parse_tokens(tokenize("(<< >> F (x>5 && y<10))", Bindings([], [], ["x", "y"])))
@test ast == _parse_tokens(tokenize("(<<>> F (x>5 && y<10))", Bindings([], [], ["x", "y"])))

ast = _parse_tokens(tokenize("not << >> F true", Bindings([], [], [])))
@test ast == StrategyUnaryOperation("not", _parse_tokens((tokenize("<< >> F true", Bindings([], [], [])))))
@test to_string(ast) == "not(<<>>F(true))"
@test ast == _parse_tokens(tokenize("(not (<< >> F (true)))", Bindings([], [], [])))

ast = _parse_tokens(tokenize("p or q and w", Bindings([], ["p", "q", "w"], [])))
...
@test_throws ParseError("Unparsed token at 'not'.") _parse_tokens(tokenize("not", Bindings([], [], [])))
@test_throws ParseError("Cannot parse tokens between '14.0' and '&&'.") _parse_tokens(tokenize("14 && true", Bindings([], [], [])))
@test_throws ParseError("Cannot parse tokens between '<<' and 'a'.") _parse_tokens(tokenize("<<a, >> F true", Bindings(["a"], [], [])))
@test_throws ParseError("Cannot parse tokens between 'true' and '&&'.") _parse_tokens(tokenize("true && false", Bindings([], [], [])), expression)
@test_throws ParseError("Cannot parse tokens between 'true' and '&&'.") _parse_tokens(tokenize("true && var", Bindings([], [], ["var"])), constraint)
