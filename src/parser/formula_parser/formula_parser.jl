...
using .Tokenizer
import .Tokenizer: to_string
export Bindings, TokenizeError
...
"""
    parse_formula(str::String, bindings::Bindings, level::ParseLevel)::Union{Strategy_Formula, State_Formula, Constraint, ExprLike, Nothing}
...
# Examples
```julia-repl
julia> parse_formula("a + b", Bindings([], [], ["a", "b"]), expression)
Add(Var(:a), Var(:b))
```
"""
function parse_formula(str::String, bindings::Bindings, level::ParseLevel)::Union{ASTNode, Nothing}
    return _parse_tokens(tokenize(str, bindings), level)
end
